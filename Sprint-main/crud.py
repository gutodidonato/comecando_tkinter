...
# INSERT
def inserir_em_tabela(nome_tabela, *valor):
    valores = ', '.join(['?'] * len(valor))
...
def consulta_especifica(nome_tabela, coluna_parametro, valor_parametro):
    cursor.execute(f'SELECT * FROM {nome_tabela} WHERE {coluna_parametro}={valor_parametro}')
    results = cursor.fetchall()
    conn.close()
    return results  
   
def achar_valor(nome_tabela, coluna_parametro, valor_parametro, coluna_parametro2, valor_parametro2, coluna_desejada, valor_desejado):
    cursor.execute(f"SELECT {coluna_desejada}  FROM {nome_tabela} WHERE {coluna_parametro} = ? AND {coluna_parametro2} = ?", (valor_parametro, valor_parametro2))
    result = cursor.fetchone()
    if result:
        valor_desejado = result[0]
